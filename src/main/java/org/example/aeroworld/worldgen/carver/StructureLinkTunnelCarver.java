...
public final class StructureLinkTunnelCarver {
    ...
    public static void carveChunk(ChunkAccess chunk, StructureLinkRegistry registry, int seaLevel) {
        ...
        // Отмечаем чанк carve'нутым В ЛЮБОМ СЛУЧАЕ (даже без сегментов) —
        // это единственный сигнал для диагностики race «структура открыта
        // позже, чем чанк на пути к ней уже сгенерирован» в registry.
        registry.markChunkCarved(chunkX, chunkZ);

        if (segments.isEmpty()) return;

        org.example.aeroworld.AeroWorld.LOGGER.debug(
                "[AeroWorld][StructureLink] Резка тоннеля в чанке ({},{}): {} сегмент(ов).",
                chunkX, chunkZ, segments.size());
